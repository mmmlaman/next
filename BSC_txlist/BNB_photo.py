...
# Load the CSV file
data_all_groups = pd.read_csv("D:/sybil/bsc_txlist_token_transfers_per_group.csv")  # Replace with your actual path

pattern = r'from:\s*"(?P<from_address>0x[a-fA-F0-9]+)".*to:\s*"(?P<to_address>0x[a-fA-F0-9]+)"'

output_directory = "BSC"
...
def plot_graph_for_column(column, index):
    ...
    ax.set_title(f"Address Relationship for BSC_txlist_group{index} with Shell Layout", fontsize=16, pad=20)

    output_path = os.path.join(output_directory, f"BSC_txlist_group{index}.png")
