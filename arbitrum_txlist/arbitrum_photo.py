...
# Load the CSV file
data_all_groups = pd.read_csv("D:/sybil/arbitrum_txlist_token_transfers_per_group.csv")  # Replace with your actual path

pattern = r'from:\s*"(?P<from_address>0x[a-fA-F0-9]+)".*to:\s*"(?P<to_address>0x[a-fA-F0-9]+)"'

output_directory = "arbitrum"
...
def plot_graph_for_column(column, index):
    ...
    ax.set_title(f"Address Relationship for arbitrum_txlist_group{index} with Shell Layout", fontsize=16, pad=20)

    output_path = os.path.join(output_directory, f"arbitrum_txlist_group{index}.png")
